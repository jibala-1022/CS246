...
int partition(int*y, int*m, int*d, int*h, int*mi, int*s, int low, int high) {
    int py = y[high];
    int pm = m[high];
    int pd = d[high];
    int ph = h[high];
    ...
    for (int j = low; j < high; j++) {
        if (py,pm,pd,ph,pmi,ps,y[j],m[j],d[j],h[j],mi[j],s[j]) {
            i++;
            swap(&py,&pm,&pd,&ph,&pmi,&ps,&y[j],&m[j],&d[j],&h[j],&mi[j],&s[j]);
        }
    }
    swap(&y[i+1],&m[i+1],&d[i+1],&h[i+1],&mi[i+1],&s[i+1],&y[high],&m[high],&d[high],&h[high],&mi[high],&s[high]);
    return (i + 1);
}

void quickSort(int*y, int*m, int*d, int*h, int*mi, int*s, int low, int high) {
    if (low < high) {
        int pi = partition(y,m,d,h,mi,s, low, high);
        quickSort(y,m,d,h,mi,s, low, pi - 1);
        quickSort(y,m,d,h,mi,s, pi + 1, high);
    }
}
...
int main(){
    ...
    for(int x=0; x<10; x++){
        ...
        quickSort(y, m, d, hr, mi, sec, 0, n-1);
